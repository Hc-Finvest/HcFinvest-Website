(function ($) {
  ...
  var contactForms = $('form[action*="assets/inc/sendmail.php"], #contact-form, #quick-contact-form, #trade-calculator-form, #contact-style1__form');
  if (contactForms.length) {
    contactForms.validate({
      submitHandler: function (form) {
        ...
        form_btn.html(form_btn.prop('disabled', true).data("loading-text") || 'Please wait...');
        $(form).ajaxSubmit({
          dataType: 'json',
          success: function (data) {
            if (data.status == 'true' || data.status === true) {
              $(form).find('.form-control, input[type="text"], input[type="email"], textarea').val('');
            }
